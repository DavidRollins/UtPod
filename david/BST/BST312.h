...
void BST_312 <ItemType>::insertItem(TreeNode*& t, const ItemType& newItem)
{
    ...
    if(newItem > t->data) insertItem(t->right, newItem);
...
{
    int count = 1;
    if(t->left != NULL) count += countNodes(t->left);
    if(t->right != NULL) count += countNodes(t->right);
    return count;
...
{
    int count = 0;
    if(this->root!= NULL)count = countNodes(this->root);
    return count;
}
...
{
    vector<ItemType> x;
    postOrderTraversal(this->root, x);
    return x;
}
...
bool BST_312 <ItemType>::isItemInTree(TreeNode* t, const ItemType& item)
{
    ...
    if(root->data < item) return isItemInTree(root->right, item);
    if(root->data > item) return isItemInTree(root->left, item);
